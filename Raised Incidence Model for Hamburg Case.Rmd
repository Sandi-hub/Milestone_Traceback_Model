...
All spatial data is stored in the coordinate system ETRS89-LAEA Europe - EPSG:3035.
...
```{r}
fit0 <- ppm(ppp_outbreak ~ offset(log(im_population)))
```
...
For one source:
$H_0: \lambda_1(x) = \rho\lambda_0(x) (1+ \alpha \ e^{-\beta x'x})$

We define the squared distance to the source:
```{r}
d2source <- function(x, y, xPOS=ppp_stores$x[1], yPOS = ppp_stores$y[1]) {
  (x - xPOS)^2 + (y - yPOS)^2}
```
